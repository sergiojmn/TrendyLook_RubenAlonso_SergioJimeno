...
public partial class Citas : ContentPage
{
    ...
    private void CrearCitas(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ExisteCitaEnFechaHora(fechaSeleccionada, horaSeleccionadaTimeSpan))
            {
                DisplayAlert("Error", "Ya existe una cita en la misma fecha y hora.", "OK");
                return;
            }
    ...
    private bool ExisteCitaEnFechaHora(DateTime fecha, TimeSpan hora)
    {
        ...
        try
        {
            using (SqlConnection conn = DatabaseConnection.GetConnection())
            {
                string query = "SELECT COUNT(*) FROM Cita WHERE CAST(Date AS DATE) = @Fecha AND Hour = @Hora";
        ...
        {
            Console.WriteLine($"Error al verificar la existencia de la cita: {ex.Message}");
        }
